...
//
// The mouse is deliberately OFF: with it on, a drag became a tmux copy-mode selection,
// which the terminal emulator cannot see (so xterm's own selection + Cmd+C never worked,
// and copying depended on a macOS-only pbcopy pipe). With it off, a drag is xterm's own
// selection. Applications that request mouse tracking themselves (vim, htop) still get
// their mouse events — tmux forwards those regardless of this option.

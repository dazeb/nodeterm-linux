...
export interface TelegramPairingCodeEvent {
  /** 6-digit numeric code the user enters in the app. */
  code: string
  /** Telegram display name of the user requesting pairing. */
  name: string
  /** Chat_id of the requesting user. */
  chatId: number
  /** Epoch ms when the code expires. */
  expiresAt: number
}

/** An approved Telegram user that can access the bot's terminal commands. */
...
export interface NodeTerminalApi {
  ...
  telegram: {
    ...
    /** Listen for a pairing-code generated by the bot, which the desktop user must approve. */
    onPairingCode(listener: (event: TelegramPairingCodeEvent) => void): () => void
    ...
    /** List approved Telegram users. */
    getApprovedUsers(): Promise<TelegramApprovedUser[]>
    /** Revoke an approved Telegram user by chat_id. */
